...
const PANEL_HEIGHT_OFFSET = 0.30 // Raise panels 30cm (~12 inches) above roof for visibility
...
function computeRoofSurfaceOrientation(position, azimuthDegrees, pitchDegrees) {
  ...
  // Normalize Y axis
  const yLen = Math.sqrt(yAxisEast * yAxisEast + yAxisNorth * yAxisNorth + yAxisUp * yAxisUp)
  ...
  const xLen = Math.sqrt(xAxisEast * xAxisEast + xAxisNorth * xAxisNorth + xAxisUp * xAxisUp)
  ...
  const rotationMatrix = new Matrix3(
    xE, yE, zE,    // Column 0: X axis in ENU
    xN, yN, zN,    // Column 1: Y axis in ENU
    xU, yU, zU     // Column 2: Z axis in ENU
  )
...
export default function RoofVisualizer3D({
  ...
  const clampPanelsToSurface = useCallback(async () => {
    ...
    console.log('Starting height sampling for', solarPanels.length, 'panels')

    // Create Cartesian3 positions for all panels
    const positions = solarPanels.slice(0, maxPanels).map(panel => {
    ...
    if (positions.length === 0) {
      console.log('No valid panel positions')
      setLoading(false)
      return
    }
    ...
    try {
      ...
      console.log('Clamped positions:', clamped.filter(p => defined(p)).length, 'of', positions.length)

      // Store clamped positions with panel data
      const panelsWithHeight = solarPanels.slice(0, maxPanels).map((panel, i) => {
        ...
        if (!defined(clampedPos)) {
          ...
          const fallbackHeight = (segment?.planeHeightAtCenterMeters || 10) + PANEL_HEIGHT_OFFSET
          return {
            ...panel,
      ...
      const fallbackPanels = solarPanels.slice(0, maxPanels).map(panel => {
        const segment = roofSegments[panel.segmentIndex]
        const height = (segment?.planeHeightAtCenterMeters || 10) + PANEL_HEIGHT_OFFSET
        return {
          ...panel,
          position: Cartesian3.fromDegrees(
            panel.center.longitude,
  ...
  const handleTilesetReady = useCallback(async (tileset) => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: 500, borderRadius: 12, overflow: 'hidden' }}>
      {/* Loading overlay */}
      {loading && (
        <div style={{
          position: 'absolute',
          ...
          <p style={{ color: 'white', margin: 0 }}>{statusMessage || 'Loading 3D Building...'}</p>
        </div>
      )}
      ...
      >
        ...
        {heightsReady && clampedPositions.slice(0, activePanelCount).map((panel, index) => {
          ...
          return (
            <Entity
              ...
              box={{
                dimensions: new Cartesian3(boxWidth, boxLength, PANEL_THICKNESS),
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{ color: 'white', minWidth: 100, textAlign: 'center' }}>
              <div style={{ fontWeight: 700, fontSize: '1.1rem' }}>{activePanelCount} Panels</div>
              <div style={{ fontSize: '0.8rem', opacity: 0.7 }}>
                {((activePanelCount * (panelDimensions?.capacityWatts || 400)) / 1000).toFixed(1)} kW
              </div>
            </div>
            <button
              onClick={() => handlePanelChange(4)}
              ...
              style={{
                ...
                padding: '8px 12px',
                fontWeight: 600,
                cursor: activePanelCount >= maxPanels ? 'not-allowed' : 'pointer',
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                background: useRealisticPanels ? 'rgba(0,0,0,0.9)' : 'linear-gradient(135deg, #22c55e, #eab308, #ef4444)',
                border: 'none',
                borderRadius: 8,
                color: 'white',
                padding: '8px 14px',
          ...
          <div style={{
            ...
            <div style={{ color: 'white', fontSize: '0.8rem', fontWeight: 600, marginBottom: 10 }}>
              Production Ranking
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 18, height: 18, background: '#22c55e', borderRadius: 4 }} />
                <span style={{ color: 'white', fontSize: '0.75rem' }}>Best</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 18, height: 18, background: '#eab308', borderRadius: 4 }} />
                <span style={{ color: 'white', fontSize: '0.75rem' }}>Good</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 18, height: 18, background: '#ef4444', borderRadius: 4 }} />
                <span style={{ color: 'white', fontSize: '0.75rem' }}>Fair</span>
              </div>
            </div>
          </div>
