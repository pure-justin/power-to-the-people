...
export interface ComplianceInput {
  ...
  project_type: "residential" | "commercial" | "tpo";
  state: string;
  installation_date?: string;
  financing_type?: "cash" | "loan" | "lease" | "ppa";
  system_cost?: number;
}

export interface EquipmentFEOCDetail {
  equipment_id: string;
  name: string;
  manufacturer: string;
  type: string;
  pfe_percentage: number;
  threshold: number;
  passes: boolean;
  reason: string;
}

export interface DomesticContentResult {
  score: number;
  threshold: number;
  eligible: boolean;
  bonus_value: string;
...
export interface ITCEligibility {
  eligible: boolean;
  base_rate: number;
  domestic_content_adder: number;
  energy_community_adder: number;
  total_rate: number;
  estimated_value: number;
  reason: string;
}

export interface TariffExposureItem {
  equipment_id: string;
  name: string;
...
export interface SingleEquipmentCompliance {
  equipment_id: string;
  name: string;
  manufacturer: string;
  type: string;
  ...
  feoc_reason: string;
  domestic_content_percentage: number;
  tariff_safe: boolean;
  tariff_details: {
    country_of_origin: string;
    tariff_type: string;
...
export interface ComplianceResult {
  feoc_status: "compliant" | "non_compliant" | "partial" | "unknown";
  feoc_details: EquipmentFEOCDetail[];
  domestic_content: DomesticContentResult;
  itc_eligibility: ITCEligibility;
  tariff_exposure: TariffExposureItem[];
...
function isKnownFEOCEntity(manufacturer: string): boolean {
  const mfr = (manufacturer || "").toLowerCase().trim();
  return KNOWN_FEOC_ENTITIES.some(
    (entity) => mfr.includes(entity) || entity.includes(mfr),
  );
}

function isFEOCCountry(countryCode: string): boolean {
  return FEOC_COUNTRIES.includes((countryCode || "").toUpperCase());
}

function getPFEThreshold(installationDate?: string): number {
  // 2026: 40%, increases 5% per year to 60% by 2029
  const year = installationDate
    ? new Date(installationDate).getFullYear()
    : new Date().getFullYear();
  if (year <= 2026) return 40;
  if (year === 2027) return 45;
  if (year === 2028) return 50;
  if (year === 2029) return 55;
  return 60;
}

function getBatteryThreshold(installationDate?: string): number {
  // 2026: 55% critical minerals, increases to 75% by 2029
  const year = installationDate
    ? new Date(installationDate).getFullYear()
    : new Date().getFullYear();
  if (year <= 2026) return 55;
  if (year === 2027) return 60;
  if (year === 2028) return 65;
  if (year === 2029) return 70;
  return 75;
}
...
function getTariffInfo(
  ...
  if (
    applicableTypes.includes((equipmentType || "").toLowerCase()) ||
    code === "MX"
  ) {
...
function extractCountryCode(equipment: any): string {
...
function getDomesticContentPct(equipment: any): number {
  ...
  if (typeof equipment.supply_chain?.domestic_content_pct === "number")
  ...
  if (equipment.domestic_content_eligible === true) {
    ...
    if (country === "US") return 70; // Conservative estimate for US-made
    return 30; // US assembly with imported components
  }

  if (extractCountryCode(equipment) === "US") return 50; // Default for US-origin
  return 0;
}
...
export async function analyzeCompliance(
  ...
  const equipmentDocs = await Promise.all(
    input.equipment_ids.slice(0, 20).map(async (id: string) => {
      const doc = await db.collection("solar_equipment").doc(id).get();
      if (!doc.exists) return { id, _notFound: true };
      return { id, ...doc.data() };
    }),
  );

  // ── FEOC Analysis ──
  const feocDetails: EquipmentFEOCDetail[] = [];
  let feocPassCount = 0;
  let feocFailCount = 0;
  let feocUnknownCount = 0;

  for (const equip of equipmentDocs) {
    if ((equip as any)._notFound) {
      feocDetails.push({
    ...
    const currentThreshold =
      eqType === "battery" ? batteryThreshold : threshold;
    const pfe =
      typeof data.supply_chain?.pfe_percentage === "number"
  ...
  let feocStatus: ComplianceResult["feoc_status"];
  if (feocFailCount === 0 && feocUnknownCount === 0) feocStatus = "compliant";
  else if (feocFailCount > 0 && feocPassCount > 0) feocStatus = "partial";
  else if (feocFailCount > 0) feocStatus = "non_compliant";
  else feocStatus = "unknown";
  ...
  for (const equip of equipmentDocs) {
    if ((equip as any)._notFound) continue;
    const data = equip as any;
  ...
  const domesticScore =
    totalWeight > 0 ? Math.round(weightedDomestic / totalWeight) : 0;
  const domesticThreshold = 50;
  const domesticEligible = domesticScore >= domesticThreshold;

  const domesticContent: DomesticContentResult = {
    score: domesticScore,
  ...
  const itcEligibility: ITCEligibility = {
  ...
  // ── Tariff Exposure ──
  const tariffExposure: TariffExposureItem[] = [];
  ...
  for (const equip of equipmentDocs) {
    ...
    if (tariff && tariff.rate > 0) {
      ...
      const unitCost =
        data.pricing?.wholesale_per_unit || data.price_estimate_usd || 0;
      ...
      tariffExposure.push({
        equipment_id: equip.id,
        name: data.name || data.model || "Unknown",
        country_of_origin: `${country} (${TARIFF_DATA[country]?.name || country})`,
  ...
  if (feocFailedItems.length > 0) {
    ...
    recommendations.push(
      `Replace FEOC non-compliant equipment (${failedNames}) with US-manufactured alternatives from QCells, Silfab, First Solar, Mission Solar, Heliene, or Enphase.`,
    );
  ...
  if (!domesticEligible && itcEligible) {
    recommendations.push(
      `Domestic content score is ${domesticScore}%, below the 50% threshold for the 10% ITC adder. Consider substituting imported components with US-manufactured alternatives.`,
    );
  }

  // Tariff recommendations
  if (tariffExposure.length > 0) {
    const totalTariffImpact = tariffExposure.reduce(
      (sum, t) => sum + t.estimated_cost_impact,
      0,
    );
    if (totalTariffImpact > 0) {
      recommendations.push(
        `Total estimated tariff cost impact: $${totalTariffImpact.toLocaleString()}. US-manufactured equipment eliminates tariff exposure entirely.`,
      );
    }
  }
  ...
  if (
    ...
    recommendations.push(
      "Residential ITC is no longer available for cash/loan purchases. Switch to lease or PPA (TPO) financing to access the 48E commercial ITC via the third-party owner.",
    );
  }

  if (itcEligible && ecAdder > 0) {
    recommendations.push(
      "Energy community adder (+10%) requires verification that the installation site is in a designated energy community. Verify at energycommunities.gov.",
    );
  }
  ...
  if (!itcEligible && feocFailedItems.length > 0) {
    recommendations.push(
      "If this project is in a market without ITC (e.g., San Antonio CPS Energy territory), FEOC compliance is not required. Cheaper non-compliant panels may be acceptable to reduce cost.",
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      "System is fully compliant. All equipment meets FEOC requirements and qualifies for available tax credits.",
    );
  }
...
export async function quickComplianceCheck(
  equipmentId: string,
): Promise<SingleEquipmentCompliance> {
  const db = admin.firestore();
  ...
  // FEOC check
  let feocCompliant: boolean;
  let feocReason: string;
