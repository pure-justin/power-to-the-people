...
export interface ProposalInput {
  ...
  rateEscalation?: number; // default 3%
  // Cost & pricing
  ...
  federalITC: number; // decimal (e.g. 0.30)
  stateIncentive: number; // dollar amount
  utilityIncentive: number; // dollar amount
  ...
  domesticContentCompliant?: boolean;
  tariffSafe?: boolean;
  // Warranties
  ...
  salesRepName?: string;
  // Proposal config
  degradationRate?: number; // default 0.5%
...
export interface Proposal {
  ...
  equipmentList: EquipmentListSection;
  productionEstimate: ProductionEstimateSection;
  financialAnalysis: FinancialAnalysisSection;
  environmentalImpact: EnvironmentalImpactSection;
  timeline: TimelineSection;
  warrantySummary: WarrantySummarySection;
  disclaimers: string[];
}
...
export interface ProposalMeta {
  ...
  validUntil: string; // ISO timestamp (30 days)
  version: string;
  customerName: string;
  customerEmail?: string;
  customerPhone?: string;
...
export interface SystemOverviewSection {
  systemSizeKw: number;
  systemSizeDc: string; // formatted, e.g. "8.40 kW DC"
  panelCount: number;
  panelType: string; // "SunPower Maxeon 6 - 420W"
  inverterType: string;
  inverterCount: number;
  hasBattery: boolean;
  batteryType?: string;
  ...
  estimatedOffsetPercentage: number;
  costPerWatt: number;
...
export interface ProductionEstimateSection {
  annualProductionKwh: number;
  monthlyProduction: Array<{
    month: string;
    productionKwh: number;
    usageKwh: number;
    netGridKwh: number; // positive = export, negative = import
  }>;
  year1ProductionKwh: number;
  year25ProductionKwh: number;
...
export interface WarrantySummarySection {
  items: WarrantyItem[];
...
export function generateProposal(input: ProposalInput): Proposal {
  const {
    ...
    ppaTermYears = 25,
    feocCompliant = true,
    domesticContentCompliant = false,
    tariffSafe = true,
  ...
  const year25ProductionKwh = Math.round(
    annualProductionKwh * Math.pow(1 - degradationRate, 24),
  );
  ...
  const recValue = calculateRECValue({
    state,
    annualProductionKwh,
  });
  ...
  const cashAnalysis = analyzeCashPurchase({
    systemCostTotal,
  ...
  if (optionCount >= 2) {
    comparison = generateFinancialComparison({
      systemCostTotal,
      annualProductionKwh,
      annualUsageKwh,
      utilityRatePerKwh,
    ...
    systemOverview: {
      ...
      batteryType:
        batteryCount && batteryCount > 0
          ...
          : undefined,
      batteryCapacityKwh:
        batteryCount && batteryCount > 0 ? batteryCapacityKwh : undefined,
      batteryCount: batteryCount && batteryCount > 0 ? batteryCount : undefined,
      rackingType,
      estimatedAnnualProductionKwh: annualProductionKwh,
      estimatedOffsetPercentage: Math.round(
        (annualProductionKwh / annualUsageKwh) * 100,
      ),
      costPerWatt,
      totalSystemCost: systemCostTotal,
      netCostAfterIncentives: Math.round(netCostAfterIncentives * 100) / 100,
      compliance: {
        feocCompliant: feocCompliant ?? true,
        domesticContentCompliant: domesticContentCompliant ?? false,
        tariffSafe: tariffSafe ?? true,
      },
    },
    equipmentList: {
...
function calculateEnvironmentalImpact(
  ...
  const equivalentTreesPlanted = Math.round(
    annualCO2Tons * TREES_PER_TON_CO2_YEAR,
  );

  // Miles not driven over lifetime
  const equivalentMilesNotDriven = Math.round(
    lifetimeCO2OffsetTons * MILES_PER_TON_CO2,
  );

  // Homes powered (based on annual production)
  const equivalentHomesPowered =
    Math.round((annualProductionKwh / AVG_HOME_KWH_PER_YEAR) * 10) / 10;

  // Gallons of gasoline saved over lifetime
  const equivalentGallonsGasoline = Math.round(
    lifetimeCO2OffsetTons * GALLONS_GAS_PER_TON_CO2,
  );

  // Smartphone charges over lifetime
  const equivalentSmartphoneCharges = Math.round(
    lifetimeProductionKwh * SMARTPHONE_CHARGES_PER_KWH,
  );

  // Coal not burned annually
  const annualCoalNotBurnedKg = Math.round(
    annualProductionKwh * COAL_KG_PER_KWH,
  );

  // Build a human-readable impact statement
  const impactStatement =
    `Over ${analysisYears} years, this solar system will offset approximately ` +
...
function buildFinancialHighlights(
  ...
  const year1Savings = cash.yearlyProjections[0]?.netSavings || 0;
  highlights.push({
  ...
  const bestSavings = Math.max(
    cash.netSavings25Year,
    loan?.netSavings25Year || -Infinity,
    lease?.totalSavings || -Infinity,
    ppa?.totalSavings || -Infinity,
  );
  highlights.push({
    label: "25-Year Net Savings",
    value: `$${Math.round(bestSavings).toLocaleString()}`,
    description:
      "Total estimated savings over the system lifetime (best financing option).",
  });
  ...
  highlights.push({
    ...
    description:
      "Time to recover the net investment through energy savings (cash purchase).",
  });

  // Monthly bill reduction
  const monthlyProductionValue = (annualProductionKwh / 12) * utilityRatePerKwh;
  const newMonthlyBill = Math.max(
    0,
    currentMonthlyBill - monthlyProductionValue,
  );
  highlights.push({
...
function buildProjectTimeline(
  ...
  const milestones: TimelineMilestone[] = [
    ...
    },
    {
      ...
      name: "Site Survey",
      description:
        "Detailed site assessment: roof measurements, electrical panel, shading analysis.",
      estimatedWeeks: "1-2",
      status: "pending",
    },
    {
      ...
      description:
        "CAD design, structural analysis, electrical single-line diagram.",
      estimatedWeeks: "1-2",
      status: "pending",
    },
    {
      ...
      name: "Permitting",
      description:
        "Submit permit application to local AHJ (Authority Having Jurisdiction).",
      estimatedWeeks: "2-6",
      status: "pending",
    },
    {
      ...
      description:
        "Order panels, inverters, racking, and electrical components.",
      estimatedWeeks: "1-3",
      status: "pending",
    },
    {
      ...
      name: "Installation",
      description:
        "Professional installation of racking, panels, inverter, and electrical connections.",
      estimatedWeeks: "1-3",
      status: "pending",
    },
    {
      ...
      name: "Inspection",
      description:
        "Municipal and/or county inspection of the completed installation.",
      estimatedWeeks: "1-2",
      status: "pending",
    },
    {
      ...
      description:
        "Apply for Permission to Operate from the utility company. Meter swap if needed.",
      estimatedWeeks: "2-4",
      status: "pending",
    },
    {
      ...
      description:
        "System turned on, monitoring configured, customer walkthrough.",
      estimatedWeeks: "0",
      status: "pending",
    },
  ];
  ...
  const items: WarrantyItem[] = [
    {
      ...
      coverage:
        "Covers manufacturing defects, materials, and workmanship of the solar panels.",
    },
    {
      component: `${params.panelManufacturer} ${params.panelModel} Panels`,
    ...
    },
    {
      ...
      coverage:
        "Covers inverter replacement or repair for manufacturing defects.",
    },
  ];

  if (
    params.batteryManufacturer &&
    params.batteryModel &&
    params.batteryCount &&
    ...
    items.push({
      ...
      coverage:
        "Covers battery defects and guarantees minimum capacity retention.",
    });
  }
 ...
 */
function buildDisclaimers(isResidential: boolean, state: string): string[] {
  ...
  if (isResidential) {
    disclaimers.push(
      "As of January 1, 2026, the federal residential solar Investment Tax Credit (Section 25D) has expired. Residential homeowners purchasing systems directly are not eligible for the federal ITC. Lease and PPA arrangements may allow the third-party owner to claim the commercial ITC (Section 48E) and pass savings to the homeowner.",
    );
  }

  disclaimers.push(
    "Net metering policies and rates are determined by your utility company and may change. This proposal uses current published rates.",
  );
 ...
 * @auth firebase
 */
export const generateSolarProposal = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      const input: ProposalInput = {
        ...
        utilityRatePerKwh:
          data.utilityRatePerKwh || project.utilityRatePerKwh || 0.13,
        netMeteringRate:
          data.netMeteringRate ||
        ...
        isResidential:
          data.isResidential ?? project.projectType !== "commercial",
        federalITC: data.federalITC ?? 0,
 ...
 * @auth firebase
 */
export const getSolarProposal = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!proposalId && !projectId) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Either proposalId or projectId is required.",
      );
    }
    ...
    try {
      ...
      if (snapshot.empty) {
        throw new functions.https.HttpsError(
          "not-found",
          `No proposals found for project ${projectId}.`,
        );
      }
