 ...
 */
function parseSegments(req: functions.https.Request): string[] {
  const raw = req.path || "/";
...
async function handleGetListings(
  ...
  await validateApiKeyFromRequest(req, ApiKeyScope.READ_MARKETPLACE);

  const db = admin.firestore();
  let query: admin.firestore.Query = db.collection("marketplace_listings");
  ...
  if (req.query.zip) {
    ...
    listings = listings.filter((l: any) => {
      const pZip =
        l.project_zip ||
        l.project_context?.zip ||
        l.project_context?.project_zip;
      return pZip === zip;
    });
  }
...
async function handleCreateListing(
  ...
  const budget =
    typeof budget_min === "number" && typeof budget_max === "number"
...
async function handleSubmitBid(
  ...
  if (listing.posted_by?.userId === apiKeyData.userId) {
    res.status(409).json({
      success: false,
  ...
  const proposedDays =
    typeof timeline_hours === "number" ? timeline_hours / 24 : undefined;

  const bidDataForScoring: Record<string, any> = {
    price: amount,
    proposed_days: proposedDays,
  };

  const bidScore = scoreBid(bidDataForScoring, listing, workerData, weights);
...
async function handleAcceptBid(
  ...
  batch.update(listingRef, {
    ...
    winning_bid: {
      bidId: bid_id,
      bidderId: bidData.bidder?.userId || bidData.bidder?.workerId,
      price: bidData.price,
...
async function handleCompleteListing(
  ...
  const isPosted = listing.posted_by?.userId === apiKeyData.userId;
  const isWorker = listing.winning_bid?.bidderId === apiKeyData.userId;
 ...
 * @scope read_workers
 */
async function handleGetWorkers(
  req: functions.https.Request,
  res: functions.Response,
): Promise<void> {
  await validateApiKeyFromRequest(req, ApiKeyScope.READ_WORKERS);

  const db = admin.firestore();
  let query: admin.firestore.Query = db.collection("workers");
  ...
  if (req.query.skill) {
    ...
    workers = workers.filter(
      (w: any) => Array.isArray(w.skills) && w.skills.includes(skill),
    );
  }
  ...
  if (req.query.state) {
    ...
    workers = workers.filter(
      (w: any) =>
        Array.isArray(w.service_areas) && w.service_areas.includes(state),
    );
  }
...
async function handleRegisterWorker(
  ...
  const { skills, zip_code, service_radius_miles, bio, certifications } =
    req.body;
 ...
 * @scope write_marketplace
 */
async function handleRateWorker(
  req: functions.https.Request,
  res: functions.Response,
  workerId: string,
  apiKeyData: ApiKey,
 ...
 * @scope read_marketplace
 */
async function handleMyActiveTasks(
  req: functions.https.Request,
  res: functions.Response,
  apiKeyData: ApiKey,
): Promise<void> {
 ...
 * @auth api_key
 */
async function marketplaceApiHandler(
  req: functions.https.Request,
  res: functions.Response,
): Promise<void> {
 ...
 */
export const marketplaceApi = functions
