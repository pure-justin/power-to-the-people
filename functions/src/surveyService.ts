...
type SurveySection = (typeof VALID_SECTIONS)[number];
 ...
 */
export const createSurvey = functions.https.onCall(async (data, context) => {
  // Require authentication
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "Must be signed in to create a survey",
    );
  }
  ...
  if (!projectId || !customerId) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "projectId and customerId are required",
    );
  }
 ...
 */
export const updateSurvey = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "Must be signed in",
  ...
  if (!surveyId || !sectionData) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "surveyId and sectionData are required",
    );
  }
  ...
  for (const section of sections) {
    if (!VALID_SECTIONS.includes(section as SurveySection)) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        `Invalid section: ${section}. Valid sections: ${VALID_SECTIONS.join(", ")}`,
      );
    }
  }
  ...
  const editableStatuses: SurveyStatus[] = [
    "draft",
    "in_progress",
    "revision_needed",
  ];
  ...
  if (!editableStatuses.includes(currentData.status)) {
    throw new functions.https.HttpsError(
      "failed-precondition",
      `Cannot edit survey in "${currentData.status}" status. Editable statuses: ${editableStatuses.join(", ")}`,
    );
  }
 ...
 */
export const submitSurvey = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "Must be signed in",
  ...
  if (surveyData.status !== "in_progress" && surveyData.status !== "draft") {
    throw new functions.https.HttpsError(
      "failed-precondition",
      `Cannot submit survey in "${surveyData.status}" status. Must be "in_progress" or "draft".`,
    );
  }
  ...
  const aiTaskRef = await db.collection("ai_tasks").add({
    ...
    input: {
      ...
      sections: Object.keys(surveyData).filter(
        (k) =>
          VALID_SECTIONS.includes(k as SurveySection) &&
  ...
  functions.logger.info("Survey submitted", {
    ...
    missingOptional: VALID_SECTIONS.filter(
      (s) => Object.keys(surveyData[s] || {}).length === 0,
    ),
  });
 ...
 * @throws {HttpsError} not-found - If survey does not exist
 */
export const getSurvey = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "Must be signed in",
 ...
 */
export const getSurveysByProject = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
 ...
 * @throws {HttpsError} not-found - If survey does not exist
 */
export const reviewSurvey = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "Must be signed in",
  ...
  if (!surveyId || !reviewResult) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "surveyId and reviewResult are required",
    );
  }
  ...
  const newStatus: SurveyStatus = reviewResult.design_ready
    ? "approved"
    : "revision_needed";
 ...
 * @throws {HttpsError} not-found - If survey does not exist
 */
export const addSurveyPhoto = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "Must be signed in",
  ...
  if (!surveyId || !photoData) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "surveyId and photoData are required",
    );
  }
  ...
  if (!photoData.type || !photoData.url) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "photoData must include type and url",
    );
  }

  if (
    !VALID_PHOTO_TYPES.includes(
      photoData.type as (typeof VALID_PHOTO_TYPES)[number],
    )
  ) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      `Invalid photo type: ${photoData.type}. Valid types: ${VALID_PHOTO_TYPES.join(", ")}`,
    );
  }
