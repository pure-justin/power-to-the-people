...
export const processWebhookRetries = functions.pubsub
  ...
  .onRun(async () => {
    ...
    console.log(
      `Webhook retries processed: ${results.processed} total, ${results.success} success, ${results.failed} failed`,
    );
    return results;
  });
 ...
 */
export const retryWebhook = functions.https.onCall(async (data, context) => {
  // Require authentication
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "Must be authenticated to retry webhooks",
    );
  }
  ...
  if (!url) {
    throw new functions.https.HttpsError(
      "failed-precondition",
      "No URL in webhook log to retry",
    );
  }
