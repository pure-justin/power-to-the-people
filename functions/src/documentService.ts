...
/** All document types the platform can generate */
type DocumentType =
  | "proposal"
  | "contract"
  | "change_order"
...
/** Document status through its lifecycle */
type DocumentStatus =
  | "draft"
  | "generating"
  | "generated"
...
/** A single signature entry on a document */
interface SignatureEntry {
  signerId: string;
  signerName: string;
  signerEmail: string;
  signerRole: "customer" | "installer" | "admin" | "witness";
  signatureImageUrl: string; // Base64 or Storage URL of drawn signature
  signedAt: FirebaseFirestore.Timestamp | FirebaseFirestore.FieldValue;
  ipAddress: string;
  userAgent: string;
  consentGiven: boolean; // Must be true — signer confirmed intent
  consentText: string; // The exact text they agreed to
}

/** Required signers for each document type */
const REQUIRED_SIGNERS: Record<DocumentType, string[]> = {
...
function generateDocumentHtml(
  ...
  switch (type) {
    ...
    case "completion_certificate":
      return generateCompletionCertHtml(
        commonStyles,
        header,
        footer,
        projectData,
    ...
    case "permit_package":
      return generatePermitPackageHtml(
        commonStyles,
        header,
        footer,
        projectData,
...
function generateCompletionCertHtml(
  styles: string,
  header: string,
  footer: string,
  data: Record<string, any>,
  ...
  return `<!DOCTYPE html><html><head>${styles}</head><body>
    ...
    <div class="highlight-box" style="text-align: center; margin: 30px 0;">
      ...
      <div style="font-size: 14px;">System Size: ${data.system?.totalKw || 0} kW | Panels: ${data.system?.panelCount || 0} | Completion Date: ${data.completionDate || new Date().toLocaleDateString()}</div>
    </div>
...
function generatePermitPackageHtml(
  styles: string,
  header: string,
  footer: string,
  data: Record<string, any>,
  ...
  return `<!DOCTYPE html><html><head>${styles}</head><body>
    ...
    <div class="section">
      ...
      <table>
        ...
        ${ahj.requirements?.fire_department_review ? `<tr><td>6</td><td>Fire Department Review Form</td><td>1</td></tr>` : ""}
      </table>
    </div>
  ...
  return `<!DOCTYPE html><html><head>${styles}</head><body>
    ...
    <div class="section">
      ...
      <div class="grid">
        <div class="field"><div class="field-label">Customer</div><div class="field-value">${data.customerName || ""}</div></div>
        <div class="field"><div class="field-label">Address</div><div class="field-value">${data.address || ""}</div></div>
        <div class="field"><div class="field-label">System Size</div><div class="field-value">${data.system?.totalKw || 0} kW</div></div>
...
export const generateDocument = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!projectId || !type) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "projectId and type are required.",
      );
    }
    ...
    const templateData: Record<string, any> = {
      ...
      address: project.address || surveyData.property?.address || "",
      roofType: surveyData.property?.roof_type || "",
      annualKwh: surveyData.utility?.annual_kwh || 0,
      monthlyBill: surveyData.utility?.avg_monthly_bill || 0,
      ...
      system: {
        totalKw: designData.system_design?.total_kw || project.systemSize || 0,
        panelCount: designData.system_design?.panel_count || 0,
        panelModel: designData.system_design?.panel_model || "",
        inverterModel: designData.system_design?.inverter_model || "",
        batteryModel: designData.system_design?.battery_model || null,
        mountingType: designData.system_design?.mounting_type || "roof",
        annualKwh: designData.system_design?.estimated_annual_kwh || 0,
        offsetPct: designData.system_design?.offset_percentage || 0,
      },
      savings: {
    ...
    // Generate the HTML
    const html = generateDocumentHtml(type as DocumentType, templateData);

    // Determine required signers
    const requiredSigners = REQUIRED_SIGNERS[type as DocumentType] || [];
    ...
    const docRef = await db.collection("documents").add({
      ...
      title: getDocumentTitle(type as DocumentType, templateData),
 ...
 */
export const getDocument = functions
 ...
 */
export const getDocumentsByProject = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    requireAuth(context);
 ...
 */
export const sendDocument = functions
  .runWith({ timeoutSeconds: 60, memory: "256MB" })
  .https.onCall(async (data, context) => {
    const uid = requireAuth(context);
    const { documentId, recipients } = data;

    if (!documentId || !recipients?.length) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "documentId and recipients are required.",
      );
    }
    ...
    if (docData.status === "voided" || docData.status === "expired") {
      throw new functions.https.HttpsError(
        "failed-precondition",
        `Cannot send a ${docData.status} document.`,
      );
    }

    await docRef.update({
      status: "sent",
      sentTo: recipients.map((r: any) => ({
        email: r.email,
        name: r.name,
        role: r.role,
 ...
 */
export const viewDocument = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    const uid = requireAuth(context);
    const { documentId } = data;
    ...
    if (!viewedBy.find((v: any) => v.userId === uid)) {
      viewedBy.push({
        userId: uid,
...
export const signDocument = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!documentId || !signatureImageUrl || !signerRole) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "documentId, signatureImageUrl, and signerRole are required.",
      );
    }
    ...
    if (docData.status === "voided" || docData.status === "expired") {
      throw new functions.https.HttpsError(
        "failed-precondition",
        `Cannot sign a ${docData.status} document.`,
      );
    }
    ...
    const alreadySigned = signatures.find(
      (s: any) => s.signerId === uid && s.signerRole === signerRole,
    );
    if (alreadySigned) {
      throw new functions.https.HttpsError(
        "already-exists",
        "You have already signed this document in this role.",
      );
    }
    ...
    const requiredSigners: string[] = docData.requiredSigners || [];
    const signedRoles = signatures.map((s: any) => s.signerRole);
    const allSigned = requiredSigners.every((role: string) =>
      signedRoles.includes(role),
    );

    // Update document status
    const newStatus: DocumentStatus = allSigned
      ? "signed"
      : signatures.length > 0
        ? "partially_signed"
        : docData.status;
 ...
 */
export const voidDocument = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    const uid = requireAuth(context);
    ...
    if (!documentId || !reason) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "documentId and reason are required.",
      );
    }
 ...
 */
export const saveDocumentTemplate = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    const uid = requireAuth(context);
    await requireAdmin(uid);
 ...
 */
export const getDocumentStats = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    requireAuth(context);
    const { projectId } = data || {};
