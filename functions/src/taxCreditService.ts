...
/** IRA credit types eligible for Section 6418 transfer */
type CreditType =
  | "itc_residential"
  | "itc_commercial"
  | "ptc"
...
type AuctionStyle = "fixed_price" | "auction" | "negotiation";

/** Verification level (1=basic, 4=maximum assurance) */
type VerificationLevel = 1 | 2 | 3 | 4;

/** Platform fee rate: 2% for Level 4 verified, up to 5% for Level 1 */
const PLATFORM_FEE_RATES: Record<VerificationLevel, number> = {
  1: 0.05,
  2: 0.04,
  3: 0.03,
  4: 0.02,
...
const VALID_CREDIT_TYPES: CreditType[] = [
  "itc_residential",
  "itc_commercial",
  "ptc",
  "domestic_content_bonus",
...
function calculateVerificationLevel(flags: {
  platformAudited: boolean;
  thirdPartyAudited: boolean;
  insured: boolean;
  escrowAvailable: boolean;
}): VerificationLevel {
  let level: VerificationLevel = 1;
  if (flags.platformAudited && flags.thirdPartyAudited) level = 2;
  if (level >= 2 && flags.insured) level = 3;
  if (level >= 3 && flags.escrowAvailable) level = 4;
  return level;
}
...
function calculateRiskScore(factors: {
  equipmentOriginCertain: boolean;
  ...
  if (factors.equipmentOriginCertain) score -= 15;
  else score += 15;

  // Documentation: complete records reduce risk
  if (factors.documentationComplete) score -= 10;
  else score += 10;

  // Installer certification: NABCEP or equivalent
  if (factors.installerCertified) score -= 10;
  else score += 5;

  // Project age: older projects with track record are lower risk
  if (factors.projectAgeMonths > 12) score -= 10;
  else if (factors.projectAgeMonths > 6) score -= 5;
  else score += 5;

  // All audit checks passing is the strongest signal
  if (factors.allChecksPass) score -= 15;
  else score += 15;
 ...
 */
function calculatePremiumRate(riskScore: number): number {
  if (riskScore <= 25) return 0.01;
  if (riskScore <= 50) return 0.015;
  if (riskScore <= 75) return 0.025;
  return 0.04;
}
 ...
 */
export const auditProjectCredits = functions
  .runWith({ timeoutSeconds: 60, memory: "512MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "Must be authenticated to audit credits",
      );
    }
    ...
    try {
      ...
      const financingType =
        project.financingType || project.financing_type || "cash";
      const projectType =
        project.projectType || project.project_type || "residential";
      let creditType: CreditType = "itc_commercial";
      ...
      const equipmentIds: string[] =
        project.equipmentIds || project.equipment_ids || [];
      const auditChecks: Record<string, unknown>[] = [
        {
          ...
          checkedBy: null,
          notes:
            "Verify all equipment passes FEOC requirements per IRA Section 40207",
        },
        {
          ...
          checkedBy: null,
          notes:
            "Calculate weighted domestic content percentage for 10% ITC adder",
        },
        {
          ...
          checkedBy: null,
          notes:
            "Verify installation address is in a designated energy community per IRA Section 48E(h)(2)",
        },
        {
          ...
          checkedBy: null,
          notes:
            "Verify prevailing wage requirements met for projects >1MW (or all for maximum base rate)",
        },
        {
          ...
          checkedBy: null,
          notes:
            "Verify all required IRS documentation is present: Form 7205, interconnection agreement, PTO letter, invoices",
        },
      ];
      ...
      const auditData: Record<string, unknown> = {
        ...
        status: "auditing" as AuditStatus,
        auditChecks,
        equipmentCompliance: {
      ...
      try {
        const taskData: Record<string, unknown> = {
          ...
          input: {
            ...
            creditType,
            context: {
              state: project.state || project.address?.state || "",
              zipCode: project.zipCode || project.address?.zip || "",
            },
          },
          output: null,
          aiAttempt: null,
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to audit project credits",
      );
    }
  });
 ...
 */
export const getAudit = functions
 ...
 */
export const getAuditsByProject = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
 ...
 */
export const certifyAudit = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    const failedChecks = checks.filter(
      (c: any) => c.status !== "pass" && c.status !== "waived",
    );

    if (failedChecks.length > 0) {
      const failedTypes = failedChecks.map((c: any) => c.checkType).join(", ");
      throw new functions.https.HttpsError(
        "failed-precondition",
        `Cannot certify: ${failedChecks.length} check(s) not passing: ${failedTypes}`,
      );
    }
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to certify audit",
      );
    }
  });
 ...
 */
export const addAuditCheck = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    if (!auditId || !checkType || !status) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "auditId, checkType, and status are required",
      );
    }
    ...
    if (!["pass", "fail", "waived"].includes(status)) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "status must be pass, fail, or waived",
      );
    }
    ...
    try {
      ...
      const newStatus: AuditStatus = allPass ? "verified" : audit.status;

      // Recalculate bonuses based on check results
      const bonuses: Record<string, unknown>[] = [];
      const dcCheck = checks.find(
        (c: any) => c.checkType === "domestic_content",
      );
      ...
      const ecCheck = checks.find(
        (c: any) => c.checkType === "energy_community",
      );
      ...
      const totalBonusRate = bonuses.reduce(
        (sum, b: any) => sum + (b.qualified ? b.rate : 0),
        0,
      );
      const totalCreditRate = (audit.baseRate || 0) + totalBonusRate;
      const creditAmount = Math.round(
        (audit.systemCost || 0) * (totalCreditRate / 100),
      );
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to add audit check",
      );
    }
  });
 ...
 */
export const assessCreditRisk = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    try {
      ...
      const feocCheck = checks.find(
        (c: any) => c.checkType === "feoc_compliance",
      );
      const docCheck = checks.find(
        (c: any) => c.checkType === "documentation_complete",
      );

      // Calculate project age
      const createdAt = audit.createdAt?.toMillis?.() || Date.now();
      const projectAgeMonths = Math.floor(
        (Date.now() - createdAt) / (30 * 24 * 60 * 60 * 1000),
      );

      const riskScore = calculateRiskScore({
        equipmentOriginCertain: feocCheck?.status === "pass",
        documentationComplete: docCheck?.status === "pass",
      ...
      const factors = [
        {
          ...
          impact: feocCheck?.status === "pass" ? "low" : "high",
          mitigated: feocCheck?.status === "pass",
          mitigation:
            feocCheck?.status === "pass"
        ...
        },
        {
          ...
          impact: docCheck?.status === "pass" ? "low" : "medium",
          mitigated: docCheck?.status === "pass",
          mitigation:
            docCheck?.status === "pass"
      ...
      const insuranceData: Record<string, unknown> = {
        ...
        coverage: null,
        status: "assessing" as InsuranceStatus,
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      };
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to assess credit risk",
      );
    }
  });
 ...
 */
export const quoteCreditInsurance = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    if (insuranceSnap.empty) {
      throw new functions.https.HttpsError(
        "not-found",
        `No risk assessment found for audit ${auditId}. Run assessCreditRisk first.`,
      );
    }
    ...
    try {
      const riskScore = insurance.riskAssessment?.riskScore || 50;
      ...
      const coverage = {
        type: "platform_guarantee" as const,
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to quote insurance",
      );
    }
  });
 ...
 * @output {{ success: boolean }}
 */
export const activateInsurance = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    if (!insuranceId || !paymentRef) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "insuranceId and paymentRef are required",
      );
    }
    ...
    if (insurance.status !== "quoted") {
      throw new functions.https.HttpsError(
        "failed-precondition",
        `Insurance is in status "${insurance.status}", expected "quoted"`,
      );
    }
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to activate insurance",
      );
    }
  });
 ...
 */
export const getInsurance = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
 ...
 */
export const createCreditListing = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    if (!auditId || !listingDetails) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "auditId and listingDetails are required",
      );
    }
    ...
    if (audit.status !== "certified") {
      throw new functions.https.HttpsError(
        "failed-precondition",
        `Audit must be certified before listing. Current status: "${audit.status}"`,
      );
    }
    ...
    try {
      ...
      const listingData: Record<string, unknown> = {
        ...
        listing: {
          ...
          discountRate,
          minimumBid:
            listingDetails.minimumBid || Math.round(askingPrice * 0.9),
        ...
        projectSummary: {
          state: project.state || project.address?.state || "Unknown",
          ...
          equipmentOrigin: audit.equipmentCompliance?.allPanelsFeocCompliant
            ? "US-compliant"
            : "Mixed",
          projectType:
            project.projectType || project.project_type || "residential",
 ...
 */
export const searchCreditListings = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data) => {
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to search listings",
      );
    }
  });
 ...
 */
export const getCreditListing = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
 ...
 */
export const makeOffer = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    if (!listingId || !offerAmount) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "listingId and offerAmount are required",
      );
    }
    ...
    if (listing.status !== "active" && listing.status !== "under_offer") {
      throw new functions.https.HttpsError(
        "failed-precondition",
        `Listing is not accepting offers (status: "${listing.status}")`,
      );
    }
    ...
    // Check minimum bid
    if (
      listing.listing?.minimumBid &&
      offerAmount < listing.listing.minimumBid
    ) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        `Offer ($${offerAmount}) is below minimum bid ($${listing.listing.minimumBid})`,
      );
    }
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to make offer",
      );
    }
  });
 ...
 * @output {{ success: boolean }}
 */
export const respondToOffer = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    if (!listingId || !offerId || !response) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "listingId, offerId, and response are required",
      );
    }
    ...
    if (!["accepted", "rejected", "countered"].includes(response)) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "response must be accepted, rejected, or countered",
      );
    }
    ...
    if (response === "countered" && !counterAmount) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "counterAmount is required when countering",
      );
    }
    ...
    if (listing.sellerId !== context.auth.uid) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Only the listing seller can respond to offers",
      );
    }
    ...
    try {
      ...
      const listingStatus: ListingStatus =
        response === "accepted" ? "pending_transfer" : listing.status;
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to respond to offer",
      );
    }
  });
 ...
 */
export const initiateCreditTransfer = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    if (!listingId || !offerId) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "listingId and offerId are required",
      );
    }
    ...
    const offer = (listing.offers || []).find(
      (o: any) => o.id === offerId && o.status === "accepted",
    );
    if (!offer) {
      throw new functions.https.HttpsError(
        "failed-precondition",
        `No accepted offer found with ID ${offerId}`,
      );
    }

    try {
      const creditAmount = listing.listing?.creditAmount || 0;
      ...
      const verificationLevel = (listing.verificationLevel?.level ||
        1) as VerificationLevel;
 ...
 * @output {{ success: boolean }}
 */
export const completeCreditTransfer = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    try {
      ...
      timeline.push({
        ...
        actor: context.auth.uid,
        notes:
          "All documents signed, escrow released, credit transfer complete.",
      });
 ...
 */
export const getCreditTransactions = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    try {
      let q: FirebaseFirestore.Query = db.collection("tax_credit_transactions");
 ...
 */
export const getCreditMarketStats = functions
  ...
  .https.onCall(async (_data, context) => {
    ...
    try {
      ...
      listingsSnap.docs.forEach((d) => {
        const l = d.data();
        totalListed++;
        totalListedValue += l.listing?.creditAmount || 0;

        if (l.status === "active" || l.status === "under_offer") {
          activeListed++;
          activeListedValue += l.listing?.creditAmount || 0;
        }

        const state = l.projectSummary?.state || "Unknown";
      ...
      txSnap.docs.forEach((d) => {
        const t = d.data();
        if (t.transfer?.status === "completed") {
          totalSold++;
          ...
          // Group by month
          const completedAt = t.transfer?.completedAt?.toDate?.();
          if (completedAt) {
            const monthKey = `${completedAt.getFullYear()}-${String(completedAt.getMonth() + 1).padStart(2, "0")}`;
            if (!byMonth[monthKey]) byMonth[monthKey] = { count: 0, volume: 0 };
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to get market stats",
      );
    }
  });
