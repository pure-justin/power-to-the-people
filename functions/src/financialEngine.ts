...
export interface SystemSizeParams {
  monthlyUsageKwh: number;
  targetOffset: number; // 0.0–1.0; e.g. 0.80 = 80%
  panelWattage: number; // watts per panel
  locationFactor: number; // kWh/kW/year from NREL solar resource data
  roofAzimuth?: number; // degrees; 180 = due south (optimal in N hemisphere)
  roofTilt?: number; // degrees; latitude-tilt is optimal
  shading?: number; // 0–1; 0 = no shade, 1 = full shade
}
...
export interface SystemSizeResult {
  ...
  panelWattage: number;
  azimuthDerateFactor: number;
  tiltDerateFactor: number;
  shadingDerateFactor: number;
  effectiveLocationFactor: number;
}
...
export interface ProductionParams {
  ...
  losses: number; // total system losses % (default ~14%)
}
...
export interface MonthlyProduction {
  ...
  solarResourceKwhPerM2PerDay: number;
}
...
export interface YearlyProjection {
  ...
  leasePayment?: number;
  ppaPayment?: number;
  degradedSystemSizeKw?: number;
}

// ─── Cash Analysis ──────────────────────────────────────────────────────────

export interface CashPurchaseParams {
  systemCostTotal: number;
  annualProductionKwh: number;
  utilityRatePerKwh: number;
  annualUsageKwh: number;
  rateEscalation: number; // default 0.03 (3%)
  federalITC: number; // 0.30 for 30% ITC; 0 if not eligible
  stateIncentive: number; // one-time dollar amount
  utilityIncentive: number; // one-time dollar amount
  netMeteringRate: number; // $/kWh for exported energy
  degradationRate: number; // default 0.005 (0.5%/yr)
  maintenanceCostAnnual: number;
  inverterReplacementYear: number; // year 12–15 typically
  inverterReplacementCost: number;
  discountRate?: number; // for NPV, default 0.05
...
export interface CashAnalysis {
  ...
  utilityIncentiveAmount: number;
  simplePaybackYears: number;
  roi25Year: number; // percentage
  totalSavings25Year: number;
  netSavings25Year: number; // savings minus system cost
...
export interface LoanPurchaseParams {
  systemCostTotal: number;
  downPayment: number;
  loanAmount: number;
  interestRate: number; // APR decimal (0.06 = 6%)
  loanTermYears: number; // 10, 15, 20, or 25
  annualProductionKwh: number;
  utilityRatePerKwh: number;
  annualUsageKwh: number;
  ...
  inverterReplacementCost: number;
  applyITCToLoan?: boolean; // apply ITC as lump-sum payment to principal
  discountRate?: number;
...
export interface LoanAnalysis {
  ...
  irr: number;
  npv: number;
  monthOnePayment: number;
  monthOneSavings: number;
  dayOneSavings: boolean; // is monthly savings > monthly loan payment from day 1?
  yearlyProjections: YearlyProjection[];
}

// ─── TPO (Lease / PPA) Analysis ─────────────────────────────────────────────

export interface TPOParams {
  tpoType: "lease" | "ppa";
  monthlyLeasePayment?: number; // for lease
  ppaRatePerKwh?: number; // for PPA
  escalationRate: number; // annual escalation of lease/PPA cost
  termYears: number; // 20–25 typically
  annualProductionKwh: number;
  utilityRatePerKwh: number;
  annualUsageKwh: number;
  rateEscalation: number; // utility rate escalation
  degradationRate?: number; // panel degradation
  buyoutOption?: {
    year: number;
    price: number;
  };
...
export interface TPOAnalysis {
  financingType: "lease" | "ppa";
  monthlyPaymentYear1: number;
  monthlyPaymentYear25: number;
  annualEscalation: number;
  termYears: number;
  totalTPOPayments: number;
  totalUtilityCostWithoutSolar: number;
  totalSavings: number;
  averageMonthlySavings: number;
  breakEvenYear: number | null; // year when cumulative savings turn positive
  buyoutOption: { year: number; price: number } | null;
  yearlyProjections: YearlyProjection[];
}
...
export interface ComparisonReport {
  doNothing: {
    ...
    yearlyUtilityCosts: number[];
  };
  cash: CashAnalysis | null;
  loan: LoanAnalysis | null;
  lease: TPOAnalysis | null;
  ppa: TPOAnalysis | null;
...
export interface RECParams {
  state: string;
  annualProductionKwh: number;
  srecMarketPrice?: number; // $/MWh if known
}

export interface RECAnalysis {
  state: string;
  hasSRECMarket: boolean;
  srecPricePerMwh: number;
  annualRECs: number; // 1 REC = 1 MWh
  annualRECValue: number;
  totalRECValue25Year: number;
  marketNotes: string;
}

// ─── Tax Benefits ───────────────────────────────────────────────────────────

export interface TaxBenefitParams {
  systemCost: number;
  isResidential: boolean;
  domesticContentBonus: boolean; // +10% ITC
  energyCommunityBonus: boolean; // +10% ITC
  lowIncomeBonus: boolean; // +10–20% ITC
  prevailingWage: boolean; // required for > 1MW to get 30% base
  state: string;
}

export interface TaxBenefits {
  federalITCRate: number; // total ITC rate (base + adders)
  federalITCBaseRate: number; // 30% for commercial, 0 for residential (2026)
  domesticContentAdder: number;
  energyCommunityAdder: number;
  lowIncomeAdder: number;
  federalITCAmount: number;
  stateTaxCreditRate: number;
  stateTaxCreditAmount: number;
 ...
 */
const SREC_MARKETS: Record<string, { pricePerMwh: number; notes: string }> = {
 ...
 */
const STATE_TAX_CREDITS: Record<
  string,
  { rate: number; maxAmount: number; notes: string }
> = {
  AZ: {
    rate: 0.25,
    maxAmount: 1000,
...
export function calculateSystemSize(
  ...
  const numberOfPanels =
    panelKw > 0 ? Math.ceil(roundedSystemSizeKw / panelKw) : 0;
  ...
  return {
    ...
    offsetPercentage: round2(
      annualUsageKwh > 0 ? annualProductionKwh / annualUsageKwh : 0,
    ),
    panelWattage,
...
export function estimateProduction(
  ...
  const basePeakSunHours = Math.max(
    3.0,
    Math.min(7.0, 8.5 - latitudeAbs * 0.1),
  );
  ...
  const annualProductionKwh =
    systemSizeKw * basePeakSunHours * 365 * performanceRatio;
  ...
  const monthlyProduction: MonthlyProduction[] = MONTHLY_IRRADIANCE_FACTORS.map(
    (baseFactor, i) => {
      ...
      const solarResourceKwhPerM2PerDay =
        systemSizeKw > 0
  ...
  const capacityFactor =
    theoreticalMax > 0 ? annualProductionKwh / theoreticalMax : 0;
 ...
 */
export function analyzeCashPurchase(params: CashPurchaseParams): CashAnalysis {
  const {
    systemCostTotal,
    annualProductionKwh,
    utilityRatePerKwh,
  ...
  const netSystemCost =
    systemCostTotal - federalITCAmount - stateIncentive - utilityIncentive;
  ...
  const cashFlows: number[] = [-netSystemCost]; // for IRR calculation
  ...
  for (let year = 1; year <= analysisYears; year++) {
    ...
    const degradedProduction =
      annualProductionKwh * Math.pow(1 - degradationRate, year - 1);

    // Escalated utility rate
    const currentUtilityRate =
      utilityRatePerKwh * Math.pow(1 + rateEscalation, year - 1);
    ...
    const solarValue =
      selfConsumedKwh * currentUtilityRate +
  ...
  const roi25Year =
    netSystemCost > 0 ? (netSavings25Year / netSystemCost) * 100 : 0;

  // LCOE: total lifetime cost / total lifetime production
  const lcoe =
    totalProductionKwh > 0 ? totalCostOfEnergy / totalProductionKwh : 0;
 ...
 */
export function analyzeLoanPurchase(params: LoanPurchaseParams): LoanAnalysis {
  const {
    systemCostTotal,
    downPayment,
    loanAmount,
  ...
  // Month-one economics
  const monthOneSavings =
    (annualProductionKwh / 12) * utilityRatePerKwh - maintenanceCostAnnual / 12;
 ...
 */
export function analyzeTPO(params: TPOParams): TPOAnalysis {
  const {
    tpoType,
  ...
  for (let year = 1; year <= analysisYears; year++) {
    ...
    if (year <= termYears) {
      if (tpoType === "lease") {
        const currentMonthlyLease =
          monthlyLeasePayment * Math.pow(1 + escalationRate, year - 1);
        annualTPOPayment = currentMonthlyLease * 12;
      } else {
        // PPA: pay per kWh produced
        const currentPPARate =
          ppaRatePerKwh * Math.pow(1 + escalationRate, year - 1);
    ...
    const energyCostWithSolar =
      remainingGridKwh * currentUtilityRate + annualTPOPayment;
  ...
  const monthlyPaymentYear1 =
    tpoType === "lease"
      ? monthlyLeasePayment
      : (annualProductionKwh * ppaRatePerKwh) / 12;
  const monthlyPaymentYear25 =
    tpoType === "lease"
 ...
 */
export function generateFinancialComparison(
  ...
  const doNothingYearlyCosts: number[] = [];
  ...
  // Cash analysis
  let cashAnalysis: CashAnalysis | null = null;
  ...
  // Loan analysis
  let loanAnalysis: LoanAnalysis | null = null;
  if (loanAmount > 0) {
  ...
  // Lease analysis
  let leaseAnalysis: TPOAnalysis | null = null;
  ...
  // PPA analysis
  let ppaAnalysis: TPOAnalysis | null = null;
  ...
  // Build summary table
  const summaryTable: ComparisonReport["summaryTable"] = [];
  ...
  if (cashAnalysis) {
    summaryTable.push({
      ...
      totalCost25Year: round2(
        cashAnalysis.netSystemCost +
  ...
  if (loanAnalysis) {
    summaryTable.push({
      ...
      monthlyCostYear1: round2(
        loanAnalysis.monthlyPayment + maintenanceCostAnnual / 12,
      ),
      totalCost25Year: round2(
        loanDownPayment +
  ...
  if (ppaAnalysis) {
    summaryTable.push({
      ...
      upfrontCost: 0,
      monthlyCostYear1: round2(
        (annualProductionKwh * (ppaRatePerKwh || 0)) / 12,
      ),
  ...
  if (options.length === 0) {
    ...
    const bestSavings = options.reduce((best, current) =>
      current.totalSavings25Year > best.totalSavings25Year ? current : best,
    );
    const bestNoUpfront = options
      .filter((o) => o.upfrontCost === 0)
      .reduce(
        (best, current) =>
          !best || current.totalSavings25Year > best.totalSavings25Year
            ? current
            : best,
        null as (typeof options)[0] | null,
      );
 ...
 */
export function calculateRECValue(params: RECParams): RECAnalysis {
  ...
  const hasSRECMarket =
    !!market || (srecMarketPrice !== undefined && srecMarketPrice > 0);
  const pricePerMwh =
    srecMarketPrice !== undefined ? srecMarketPrice : market?.pricePerMwh || 0;
 ...
 */
export function calculateTaxBenefits(params: TaxBenefitParams): TaxBenefits {
  const {
    systemCost,
    isResidential,
  ...
  if (!isResidential) {
    ...
    if (!prevailingWage) {
      notes.push(
        "Without prevailing wage compliance, ITC drops to 6% base for systems > 1MW.",
      );
    }
  } else {
    // Residential: ITC expired Jan 1, 2026
    notes.push(
      "Residential ITC (Section 25D) expired January 1, 2026. " +
  ...
  if (!isResidential || federalITCBaseRate > 0) {
    if (domesticContentBonus) {
      ...
      notes.push(
        `Domestic content adder: +${(domesticContentAdder * 100).toFixed(0)}% ` +
    ...
    if (energyCommunityBonus) {
      ...
      notes.push(
        `Energy community adder: +${(energyCommunityAdder * 100).toFixed(0)}% ` +
    ...
    if (lowIncomeBonus) {
      ...
      notes.push(
        `Low-income community adder: +${(lowIncomeAdder * 100).toFixed(0)}% ` +
          "(qualifying low-income / environmental justice community).",
      );
    }
  }

  const federalITCRate =
    federalITCBaseRate +
  ...
  if (stateCredit) {
    if (stateCredit.rate > 0) {
      ...
      stateTaxCreditAmount = Math.min(
        systemCost * stateCredit.rate,
        stateCredit.maxAmount,
      );
    ...
    notes.push(
      `${stateUpper}: No specific state solar tax credit found. ` +
 ...
 * @auth firebase
 */
export const calculateSolarSystemSize = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!monthlyUsageKwh || !panelWattage || !locationFactor) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "monthlyUsageKwh, panelWattage, and locationFactor are required.",
      );
    }

    const result = calculateSystemSize({
      monthlyUsageKwh,
      targetOffset: targetOffset ?? 0.8,
      panelWattage,
      locationFactor,
      roofAzimuth,
      roofTilt,
 ...
 * @auth firebase
 */
export const estimateSolarProduction = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    if (!systemSizeKw || latitude === undefined || longitude === undefined) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "systemSizeKw, latitude, and longitude are required.",
      );
    }
 ...
 * @auth firebase
 */
export const runFinancialAnalysis = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!financingType) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "financingType is required: 'cash', 'loan', 'lease', 'ppa', or 'comparison'.",
      );
    }
    ...
    try {
      switch (financingType) {
        ...
        default:
          throw new functions.https.HttpsError(
            "invalid-argument",
            `Unknown financing type: ${financingType}. Use 'cash', 'loan', 'lease', 'ppa', or 'comparison'.`,
          );
      }
    } catch (error: any) {
 ...
 * @auth firebase
 */
export const calculateSolarRECValue = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    if (!state || !annualProductionKwh) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "state and annualProductionKwh are required.",
      );
    }
 ...
 * @auth firebase
 */
export const calculateSolarTaxBenefits = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
    ...
    if (!systemCost || !state) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "systemCost and state are required.",
      );
    }
    ...
    const result = calculateTaxBenefits({
      ...
      domesticContentBonus: domesticContentBonus ?? false,
      energyCommunityBonus: energyCommunityBonus ?? false,
      lowIncomeBonus: lowIncomeBonus ?? false,
      prevailingWage: prevailingWage ?? false,
      state,
    });
