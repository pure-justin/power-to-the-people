...
/** Penalty result from a strike */
interface StrikePenalty {
  penalty: string;
  suspendedUntil?: Date;
}

/** Full SLA status for a worker */
interface WorkerSlaStatus {
  strikes: number;
...
/** Result of an SLA deadline check */
interface SlaDeadlineCheck {
  overdue: boolean;
  hoursOverdue: number;
  graceExpired: boolean;
}

/** Input data for recording a violation */
 ...
 */
export function checkSlaDeadline(
  taskType: string,
  acceptedAt: Date,
  scheduledAt?: Date,
): SlaDeadlineCheck {
 ...
 */
async function loadSlaRules(): Promise<Record<string, number>> {
 ...
 */
export async function recordViolation(data: ViolationInput): Promise<string> {
  const { workerId, listingId, projectId, taskId, type, reportedBy, details } =
    data;
  ...
  if (penalty.suspendedUntil) {
    ...
    workerUpdate.suspended_until = admin.firestore.Timestamp.fromDate(
 ...
 */
export async function applyStrikePenalty(
  workerId: string,
  strikeNumber: number,
): Promise<StrikePenalty> {
  const workerRef = db.collection("workers").doc(workerId);
  const now = new Date();
 ...
 */
export async function autoRequeue(
  listingId: string,
  workerId: string,
  reason: string,
): Promise<void> {
 ...
 */
export async function getWorkerSlaStatus(
  workerId: string,
): Promise<WorkerSlaStatus> {
  // Get worker document
  const workerSnap = await db.collection("workers").doc(workerId).get();
  ...
  if (suspended && workerData.suspended_until) {
    const suspendedUntilTs =
      workerData.suspended_until as FirebaseFirestore.Timestamp;
