...
/** Dependency status for a pipeline task */
type DependencyStatus = "blocked" | "ready" | "open" | "assigned" | "completed";

/** A single pipeline task with dependency tracking */
...
export async function createPipelineTasks(
  ...
  const hasBattery =
    projectData?.has_battery === true ||
    projectData?.system?.has_battery === true ||
    projectData?.battery === true;
 ...
 */
export async function openNextTasks(projectId: string): Promise<string[]> {
  const tasksRef = db.collection(`projects/${projectId}/pipeline_tasks`);
  ...
  allTasksSnap.forEach((doc) => {
    const task = doc.data() as PipelineTask;
  ...
  allTasksSnap.forEach((doc) => {
    ...
    const allDepsMet = task.depends_on.every(
      (dep) => statusMap[dep] === "completed",
    );
 ...
 */
export async function onPipelineTaskCompleted(
  projectId: string,
  taskType: string,
): Promise<void> {
  ...
  if (projectSnap.exists) {
    ...
    const currentPhase = projectData?.status || projectData?.pipeline_stage;
...
export async function getPipelineStatus(
  ...
  const tasks: (PipelineTask & { id: string })[] = [];
 ...
 */
async function createMarketplaceListingForTask(
  projectId: string,
  taskType: string,
): Promise<void> {
  const taskRef = db.doc(`projects/${projectId}/pipeline_tasks/${taskType}`);
  ...
  const listingData: Record<string, unknown> = {
    ...
    project_context: {
      ...
      system_size: projectData?.systemSize || projectData?.system_size || null,
      panel_count: projectData?.panelCount || projectData?.panel_count || null,
    },
    status: "open",
    bid_count: 0,
    winning_bid: null,
 ...
 */
function getDefaultDeliverables(taskType: string): string[] {
