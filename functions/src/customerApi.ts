 ...
 */
function getPublicStageName(stage: string): string {
 ...
 */
function getEstimatedTimeline(stage: string): string {
...
function getNextStep(stage: string): string {
  const nextStepMap: Record<string, string> = {
    ...
    site_survey:
      "Our team is evaluating your property for optimal solar placement",
    design: "Our engineers are designing your custom solar system",
    proposal:
      "Review your personalized proposal and let us know if you have questions",
    contract: "Sign your contract to lock in your pricing",
    permitting:
      "We're handling all permit applications with your local authority",
    installation:
      "Our crew will install your solar system on the scheduled date",
...
export const customerApi = functions
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      ...
      const statusMatch = urlPath.match(
        /^customer\/project-status\/([A-Z0-9]{6})$/i,
      );
...
async function handleSignup(
  ...
  if (!existingLead.empty) {
    ...
    res.status(200).json({
      ...
      message:
        "We already have your information. We'll be in touch within 24 hours.",
    });
    return;
  }
...
async function handleProjectStatus(
  ...
  if (leadData.projectId) {
    ...
    if (projectDoc.exists) {
      ...
      const projectStage =
        projectData.stage || projectData.status || currentStage;
...
async function handleSurveyPhotos(
  req: functions.https.Request,
  res: functions.Response,
): Promise<void> {
  ...
  const uploadUrls = await Promise.all(
    photos.map(async (photo: { name: string; content_type: string }) => {
    ...
    .set(
      {
        submitted_by: decodedToken.uid,
        submitted_at: admin.firestore.FieldValue.serverTimestamp(),
        preferred_dates: preferred_dates.map((pref: any) => ({
          date: pref.date,
