...
/** Permit types matching common solar installation requirements */
export type PermitType = "solar" | "electrical" | "building" | "hoa";
...
/** Timeline entry actor — who caused the status change */
type TimelineActor = "ai" | "human" | "ahj";
...
const VALID_STATUSES: PermitStatus[] = [
  "preparing",
  "submitting",
  "submitted",
  "under_review",
 ...
 */
export const createPermit = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "Must be authenticated to create permits",
      );
    }
    ...
    if (!projectId || !ahjId || !type) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "projectId, ahjId, and type are required",
      );
    }
    ...
    if (!VALID_PERMIT_TYPES.includes(type)) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        `Invalid permit type: "${type}". Must be one of: ${VALID_PERMIT_TYPES.join(", ")}`,
      );
    }
    ...
    try {
      const permitData: Record<string, unknown> = {
        ...
        ahjId,
        type,
        status: "preparing" as PermitStatus,
        submission: {
          method: null,
          submitted_at: null,
          submitted_by: null,
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to create permit",
      );
    }
  });
 ...
 */
export const submitPermit = functions
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "Must be authenticated to submit permits",
      );
    }
    ...
    if (
      ...
      throw new functions.https.HttpsError(
        "failed-precondition",
        `Permit is in status "${permit.status}" and cannot be submitted. Only "preparing" or "corrections_needed" permits can be submitted.`,
      );
    }
    ...
    try {
      ...
      await permitRef.update({
        ...
        timeline: admin.firestore.FieldValue.arrayUnion(
          buildTimelineEntry(
            "submitting",
            "human",
            `Submission initiated by user ${context.auth.uid}`,
          ),
        ),
        updated_at: admin.firestore.FieldValue.serverTimestamp(),
      });
      ...
      const aiTaskData: Record<string, unknown> = {
        type: "permit_submit",
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to submit permit",
      );
    }
  });
 ...
 */
export const updatePermitStatus = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "Must be authenticated to update permit status",
      );
    }
    ...
    if (!permitId || !status) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "permitId and status are required",
      );
    }
    ...
    try {
      const actor = (details?.actor as TimelineActor) || "human";
      const notes = details?.notes || `Status updated to ${status}`;

      const updateData: Record<string, unknown> = {
        status,
        timeline: admin.firestore.FieldValue.arrayUnion(
          buildTimelineEntry(status, actor, notes),
        ),
        updated_at: admin.firestore.FieldValue.serverTimestamp(),
      };
      ...
      if (status === "approved") {
        ...
        if (details?.inspection_required !== undefined) {
      ...
      if (status === "under_review" && details?.reviewer_name) {
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to update permit status",
      );
    }
  });
 ...
 */
export const getPermit = functions
  .runWith({ timeoutSeconds: 10, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "Must be authenticated to view permits",
      );
    }
 ...
 */
export const getPermitsByProject = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
 ...
 */
export const addPermitCorrection = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "Must be authenticated to add corrections",
      );
    }

    const { permitId, correction } = data;

    if (!permitId || !correction?.item || !correction?.description) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "permitId, correction.item, and correction.description are required",
      );
    }
    ...
    try {
      ...
      await permitRef.update({
        ...
        "review.corrections_requested":
          admin.firestore.FieldValue.arrayUnion(correctionEntry),
        timeline: admin.firestore.FieldValue.arrayUnion(
          buildTimelineEntry(
            "corrections_needed",
            "ahj",
            `Correction requested: ${correction.item}`,
          ),
        ),
        updated_at: admin.firestore.FieldValue.serverTimestamp(),
      });
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to add correction",
      );
    }
  });
 ...
 */
export const resolveCorrection = functions
  .runWith({ timeoutSeconds: 15, memory: "256MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
        "Must be authenticated to resolve corrections",
      );
    }
    ...
    if (!permitId || !correctionId || !resolution) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "permitId, correctionId, and resolution are required",
      );
    }
    ...
    try {
      ...
      const updatedCorrections = corrections.map(
      ...
      const allResolved = updatedCorrections.every(
      ...
      await permitRef.update({
        ...
        timeline: admin.firestore.FieldValue.arrayUnion(
          buildTimelineEntry(
            "corrections_needed",
            "human",
            `Correction resolved: ${correctionId} — ${resolution}`,
          ),
        ),
        updated_at: admin.firestore.FieldValue.serverTimestamp(),
      });
      ...
      throw new functions.https.HttpsError(
        "internal",
        error.message || "Failed to resolve correction",
      );
    }
  });
 ...
 */
export const checkPermitStatuses = functions
