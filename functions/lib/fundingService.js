...
exports.generateBankabilityPackage = exports.getFundingByProject = exports.requestMilestonePayment = exports.updateFundingStatus = exports.submitFunding = exports.checkDocumentReadiness = exports.createFundingPackage = void 0;
 ...
 */
exports.createFundingPackage = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to create funding packages");
    }
    const { projectId, type, provider, fundingAmount } = data;
    if (!projectId || !type || !provider) {
        throw new functions.https.HttpsError("invalid-argument", "projectId, type, and provider are required");
    }
    if (!VALID_FUNDING_TYPES.includes(type)) {
        throw new functions.https.HttpsError("invalid-argument", `Invalid funding type: "${type}". Must be one of: ${VALID_FUNDING_TYPES.join(", ")}`);
    }
    try {
        ...
        functions.logger.info(`Funding package created: ${ref.id} (type=${type}, provider=${provider}, project=${projectId})`);
        ...
        if (error instanceof functions.https.HttpsError)
            throw error;
        throw new functions.https.HttpsError("internal", error.message || "Failed to create funding package");
    }
});
 ...
 * @firestore funding_packages
 */
exports.checkDocumentReadiness = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to check documents");
    }
    ...
    try {
        ...
        if (!packageSnap.exists) {
            throw new functions.https.HttpsError("not-found", `Funding package not found: ${packageId}`);
        }
        ...
        if (error instanceof functions.https.HttpsError)
            throw error;
        throw new functions.https.HttpsError("internal", error.message || "Failed to check documents");
    }
});
 ...
 */
exports.submitFunding = functions
    .runWith({ timeoutSeconds: 30, memory: "512MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to submit funding");
    }
    const { packageId } = data;
    if (!packageId) {
        throw new functions.https.HttpsError("invalid-argument", "packageId is required");
    ...
    try {
        ...
        if (pkg.status === "preparing") {
            ...
            const missing = REQUIRED_DOCUMENTS.filter((doc) => !documents[doc]);
            if (missing.length > 0) {
                throw new functions.https.HttpsError("failed-precondition", `Documents not ready. Missing: ${missing.join(", ")}`);
            }
        }
        ...
        functions.logger.info(`Funding submitted: package=${packageId}, task=${taskRef.id}`);
        return {
            success: true,
            status: "submitted",
            taskId: taskRef.id,
        ...
        if (error instanceof functions.https.HttpsError)
            throw error;
        throw new functions.https.HttpsError("internal", error.message || "Failed to submit funding");
    }
});
 ...
 * @firestore funding_packages
 */
exports.updateFundingStatus = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to update funding status");
    }
    const { packageId, status, details } = data;
    if (!packageId || !status) {
        throw new functions.https.HttpsError("invalid-argument", "packageId and status are required");
    }
    const validStatuses = [
        "preparing",
        "documents_ready",
    ...
    if (!validStatuses.includes(status)) {
        throw new functions.https.HttpsError("invalid-argument", `Invalid status: "${status}". Must be one of: ${validStatuses.join(", ")}`);
    }
    try {
        const packageRef = db.collection("funding_packages").doc(packageId);
        const packageSnap = await packageRef.get();
        ...
        if (status === "funded" && (details === null || details === void 0 ? void 0 : details.referenceNumber)) {
            updateData["submission.reference_number"] = details.referenceNumber;
        }
        await packageRef.update(updateData);
        functions.logger.info(`Funding status updated: package=${packageId}, status=${status}`);
        ...
        if (error instanceof functions.https.HttpsError)
            throw error;
        throw new functions.https.HttpsError("internal", error.message || "Failed to update funding status");
    }
});
 ...
 * @firestore funding_packages
 */
exports.requestMilestonePayment = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to request payments");
    }
    const { packageId, milestone } = data;
    if (!packageId || !milestone) {
        throw new functions.https.HttpsError("invalid-argument", "packageId and milestone are required");
    }
    try {
        const packageRef = db.collection("funding_packages").doc(packageId);
        const packageSnap = await packageRef.get();
        ...
        const milestoneIndex = milestones.findIndex((m) => m.milestone === milestone);
        if (milestoneIndex === -1) {
            throw new functions.https.HttpsError("not-found", `Milestone not found: ${milestone}`);
        }
        ...
        functions.logger.info(`Milestone payment requested: package=${packageId}, milestone=${milestone}`);
        ...
        if (error instanceof functions.https.HttpsError)
            throw error;
        throw new functions.https.HttpsError("internal", error.message || "Failed to request milestone payment");
    }
});
 ...
 * @firestore funding_packages
 */
exports.getFundingByProject = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to view funding");
    }
    ...
    try {
        ...
        if (error instanceof functions.https.HttpsError)
            throw error;
        throw new functions.https.HttpsError("internal", error.message || "Failed to get funding packages");
    }
});
 ...
 */
exports.generateBankabilityPackage = functions
    .runWith({ timeoutSeconds: 60, memory: "512MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to generate bankability packages");
    }
    ...
    try {
        ...
        const production = {
            ...
            annual_kwh: (projectData === null || projectData === void 0 ? void 0 : projectData.systemSize)
        ...
        if (production.annual_kwh > 0) {
            ...
            for (let year = 0; year < 25; year++) {
                production.twenty_five_year_kwh.push(Math.round(production.annual_kwh *
        ...
        const financialModel = {
            system_cost: finData.systemCost || (projectData === null || projectData === void 0 ? void 0 : projectData.systemCost) || 0,
        ...
        functions.logger.info(`Bankability package generated: ${ref.id} for project ${projectId}`);
        ...
        if (error instanceof functions.https.HttpsError)
            throw error;
        throw new functions.https.HttpsError("internal", error.message || "Failed to generate bankability package");
    }
});
