...
function parseRelativeSegments(segments) {
 ...
 * GET /projects/:id — Get project detail + tasks + timeline
 */
async function handleGetProject(_req, res, userId, projectId) {
    const db = admin.firestore();
    ...
    // Get pipeline tasks
    const pipelineStatus = await (0, pipelineAutoTasks_1.getPipelineStatus)(projectId);
    // Get timeline
 ...
 * POST /projects/:id/advance — Advance pipeline stage
 */
async function handleAdvanceStage(req, res, userId, projectId) {
    const db = admin.firestore();
    const projectRef = db.collection("projects").doc(projectId);
    const projectSnap = await projectRef.get();
    if (!projectSnap.exists) {
 ...
 * GET /projects/:id/tasks — List tasks with marketplace status
 */
async function handleListTasks(_req, res, userId, projectId) {
    const db = admin.firestore();
 ...
 * POST /projects/:id/tasks — Create manual task
 */
async function handleCreateTask(req, res, userId, projectId) {
    var _a;
    const db = admin.firestore();
    // Verify project access
    const projectSnap = await db.collection("projects").doc(projectId).get();
    ...
    const dependsOnList = Array.isArray(depends_on) ? depends_on : [];
    ...
    if (dependsOnList.length > 0) {
        ...
        for (const depType of dependsOnList) {
            const depSnap = await tasksRef.doc(depType).get();
            if (depSnap.exists && ((_a = depSnap.data()) === null || _a === void 0 ? void 0 : _a.dependency_status) !== "completed") {
                allMet = false;
                break;
            }
        }
 ...
 * POST /projects/:id/tasks/:tid/complete — Complete task
 */
async function handleCompleteTask(req, res, userId, projectId, taskId) {
    const db = admin.firestore();
    // Verify project access
    const projectSnap = await db.collection("projects").doc(projectId).get();
    if (!projectSnap.exists) {
    ...
    await (0, pipelineAutoTasks_1.onPipelineTaskCompleted)(projectId, taskId);
 ...
 * GET /projects/:id/timeline — Get full project timeline
 */
async function handleGetTimeline(_req, res, userId, projectId) {
    var _a;
    const db = admin.firestore();
    // Verify project access
    const projectSnap = await db.collection("projects").doc(projectId).get();
    ...
    const projectData = projectSnap.data();
    const inlineTimeline = ((_a = projectData.pipeline) === null || _a === void 0 ? void 0 : _a.timeline) || [];
    res.status(200).json({
        success: true,
        projectId,
 ...
 * POST /projects/:id/equipment — Set equipment selection
 */
async function handleSetEquipment(req, res, userId, projectId) {
    const db = admin.firestore();
    const projectRef = db.collection("projects").doc(projectId);
    const projectSnap = await projectRef.get();
    if (!projectSnap.exists) {
    ...
    if (!panels && !inverter && !battery && !mounting) {
        res.status(400).json({
            error: "At least one equipment field is required: panels, inverter, battery, mounting",
        });
        return;
    }
    ...
    const equipmentIds = [];
    if (panels)
        equipmentIds.push(panels);
    if (inverter)
        equipmentIds.push(inverter);
    if (battery)
        equipmentIds.push(battery);
    if (mounting)
        equipmentIds.push(mounting);
    const equipmentDocs = {};
    ...
    if (panels) {
        equipmentSelection.panels = {
            ...
            domestic_content_eligible: equipmentDocs[panels].domestic_content_eligible || false,
        };
    }
    if (inverter) {
...
exports.projectApi = functions
    ...
    try {
        ...
        const requiredScope = isWrite
            ? apiKeys_1.ApiKeyScope.WRITE_PROJECTS
            : apiKeys_1.ApiKeyScope.READ_PROJECTS;
        // Authenticate
        const apiKeyData = await (0, apiKeys_1.validateApiKeyFromRequest)(req, requiredScope);
        ...
        if (route.projectId &&
            ...
            req.method === "POST") {
            await handleCompleteTask(req, res, userId, route.projectId, route.taskId);
            return;
        }
