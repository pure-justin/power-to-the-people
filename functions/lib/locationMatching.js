...
exports.seedZipCoordinates = void 0;
exports.getZipCoordinates = getZipCoordinates;
exports.haversineDistance = haversineDistance;
exports.findWorkersInRange = findWorkersInRange;
...
async function findWorkersInRange(projectZip, serviceType, radiusMiles = DEFAULT_RADIUS_MILES) {
    ...
    for (const doc of snapshot.docs) {
        ...
        if (worker.suspended_until) {
            const suspendedUntil = worker.suspended_until;
        ...
        if (workerLat === undefined || workerLng === undefined) {
            if (Array.isArray(worker.service_areas) &&
        ...
        const distance = haversineDistance(projectCoords.lat, projectCoords.lng, workerLat, workerLng);
        // Respect the worker's own service radius if set
        const workerRadius = typeof worker.service_radius_miles === "number"
 ...
 */
exports.seedZipCoordinates = functions.https.onCall(async (_data, context) => {
    var _a;
    if (!context.auth) {
