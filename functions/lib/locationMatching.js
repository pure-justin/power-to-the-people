...
exports.lookupZipCoordinates = exports.bulkSeedZipCoordinates = exports.findNearbyWorkers = void 0;
 ...
 */
function approximateZipCoordinates(zip) {
 ...
 */
async function findWorkersInRange(projectZip, serviceType, radiusMiles = DEFAULT_RADIUS_MILES) {
    var _a;
    ...
    for (const doc of snapshot.docs) {
        ...
        const distance = haversineDistance(projectCoords.lat, projectCoords.lng, workerCoords.lat, workerCoords.lng);
        if (distance <= radiusMiles) {
            matches.push({
                workerId: doc.id,
                workerName: worker.name || "Unknown",
                distance: Math.round(distance * 10) / 10,
                rating: ((_a = worker.ratings) === null || _a === void 0 ? void 0 : _a.overall) || 0,
 ...
 */
async function seedZipCoordinates(zipData) {
 ...
 */
exports.findNearbyWorkers = functions
    ...
    if (!projectZip || !serviceType) {
        throw new functions.https.HttpsError("invalid-argument", "projectZip and serviceType are required");
    }
    if (!/^\d{5}$/.test(projectZip)) {
        throw new functions.https.HttpsError("invalid-argument", "projectZip must be a valid 5-digit US zip code");
    }
 ...
 */
exports.bulkSeedZipCoordinates = functions
    .runWith({ timeoutSeconds: 30, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be signed in");
    ...
    if ((userData === null || userData === void 0 ? void 0 : userData.role) !== "admin" && !context.auth.token.admin) {
        throw new functions.https.HttpsError("permission-denied", "Only admins can seed zip coordinate data");
    }
    const { zipData } = data;
    if (!zipData || typeof zipData !== "object") {
        throw new functions.https.HttpsError("invalid-argument", "zipData must be an object mapping zip codes to coordinates");
    }
 ...
 */
exports.lookupZipCoordinates = functions
    .runWith({ timeoutSeconds: 30, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be signed in");
    ...
    const { zip } = data;
    if (!zip || !/^\d{5}$/.test(zip)) {
        throw new functions.https.HttpsError("invalid-argument", "zip must be a valid 5-digit US zip code");
    }
...
//# sourceMappingURL=locationMatching.js.map