...
exports.getCustomerSchedule = exports.getUpcomingInstalls = exports.getInstallSchedule = exports.reschedule = exports.confirmSchedule = exports.proposeSchedule = exports.getAvailability = exports.setAvailability = void 0;
 ...
 */
exports.setAvailability = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to set availability");
    }
    const { installerId, date, timeSlots, crewSize, serviceAreaMiles, equipmentAvailable, } = data;
    if (!installerId || !date || !timeSlots || !crewSize) {
        throw new functions.https.HttpsError("invalid-argument", "installerId, date, timeSlots, and crewSize are required");
    }
    ...
    if (!Array.isArray(timeSlots) || timeSlots.length === 0) {
        throw new functions.https.HttpsError("invalid-argument", "timeSlots must be a non-empty array of {start, end} objects");
    }
    try {
...
exports.getAvailability = functions
    ...
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to view availability");
    }
    const { installerId, startDate, endDate } = data;
    if (!installerId || !startDate || !endDate) {
        throw new functions.https.HttpsError("invalid-argument", "installerId, startDate, and endDate are required");
    }
    try {
 ...
 */
exports.proposeSchedule = functions
    .runWith({ timeoutSeconds: 60, memory: "512MB" })
    .https.onCall(async (data, context) => {
    var _a, _b, _c;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to propose schedules");
    }
    const { projectId, permitId, preferredDates, preferredTimeOfDay } = data;
    if (!projectId || !permitId) {
        throw new functions.https.HttpsError("invalid-argument", "projectId and permitId are required");
    }
    try {
        ...
        const availableSlots = slotsSnapshot.docs
            ...
            .filter((slot) => { var _a; return (_a = slot.time_slots) === null || _a === void 0 ? void 0 : _a.some((ts) => ts.status === "available"); });
        ...
        const proposedSlots = availableSlots
            ...
            .slice(0, 3)
            .map((slot) => {
            const availableWindow = slot.time_slots.find((ts) => ts.status === "available");
            return {
        ...
        const scheduleData = {
            projectId,
            permitId,
            installerId: ((_a = proposedSlots[0]) === null || _a === void 0 ? void 0 : _a.installerId) || null,
            date: ((_b = proposedSlots[0]) === null || _b === void 0 ? void 0 : _b.date) || null,
            time_window: ((_c = proposedSlots[0]) === null || _c === void 0 ? void 0 : _c.time_window) || null,
            status: "proposed",
...
exports.confirmSchedule = functions
    ...
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to confirm schedules");
    }
    const { scheduleId, confirmedBy } = data;
    if (!scheduleId || !confirmedBy) {
        throw new functions.https.HttpsError("invalid-argument", "scheduleId and confirmedBy are required");
    }
    if (!["customer", "installer"].includes(confirmedBy)) {
        throw new functions.https.HttpsError("invalid-argument", 'confirmedBy must be "customer" or "installer"');
    }
    try {
        const scheduleRef = db.collection("install_schedules").doc(scheduleId);
        const scheduleSnap = await scheduleRef.get();
        if (!scheduleSnap.exists) {
            throw new functions.https.HttpsError("not-found", `Schedule not found: ${scheduleId}`);
        }
        ...
        if (newStatus === "both_confirmed" &&
            ...
            if (!slotQuery.empty) {
                ...
                const updatedSlots = (slotData.time_slots || []).map((ts) => {
...
exports.reschedule = functions
    ...
    if (!scheduleId || !newDate || !reason) {
        throw new functions.https.HttpsError("invalid-argument", "scheduleId, newDate, and reason are required");
    }
    try {
        const scheduleRef = db.collection("install_schedules").doc(scheduleId);
        const scheduleSnap = await scheduleRef.get();
 ...
 * @firestore install_schedules
 */
exports.getInstallSchedule = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to view schedules");
    }
 ...
 * @firestore install_schedules
 */
exports.getUpcomingInstalls = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to view installs");
    }
    const { installerId, limit: queryLimit } = data;
    if (!installerId) {
        throw new functions.https.HttpsError("invalid-argument", "installerId is required");
    }
    try {
 ...
 */
exports.getCustomerSchedule = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to view schedules");
