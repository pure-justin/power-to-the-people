...
exports.calculateSystemSize = exports.approveDesign = exports.updateDesign = exports.getDesignsByProject = exports.getDesign = exports.generateDesign = void 0;
 ...
 */
exports.generateDesign = functions
    .runWith({ timeoutSeconds: 30, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to generate designs");
    }
    const { projectId, surveyId } = data;
    if (!projectId || !surveyId) {
        throw new functions.https.HttpsError("invalid-argument", "projectId and surveyId are required");
    }
 ...
 */
exports.getDesign = functions
    .runWith({ timeoutSeconds: 10, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to view designs");
    }
    const { designId } = data;
    if (!designId) {
    ...
    if (!designSnap.exists) {
        throw new functions.https.HttpsError("not-found", `Design not found: ${designId}`);
    }
    return {
        success: true,
 ...
 */
exports.getDesignsByProject = functions
 ...
 */
exports.updateDesign = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to update designs");
    }
    const { designId, changes } = data;
    if (!designId || !changes) {
        throw new functions.https.HttpsError("invalid-argument", "designId and changes are required");
    }
    const db = admin.firestore();
 ...
 * @output {{ success: boolean, designId: string }}
 */
exports.approveDesign = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to approve designs");
    }
    const { designId } = data;
    if (!designId) {
        throw new functions.https.HttpsError("invalid-argument", "designId is required");
    ...
    if (design.status === "approved") {
        throw new functions.https.HttpsError("failed-precondition", "Design is already approved");
    }
    try {
        await designRef.update({
            status: "approved",
            "human_review.reviewer_id": context.auth.uid,
            "human_review.reviewed_at": admin.firestore.FieldValue.serverTimestamp(),
 ...
 */
exports.calculateSystemSize = functions
    .runWith({ timeoutSeconds: 10, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated to calculate system size");
    }
    const { annualKwh, usableRoofSqft, shadingLoss, sunHours } = data;
    if (!annualKwh || !usableRoofSqft) {
        throw new functions.https.HttpsError("invalid-argument", "annualKwh and usableRoofSqft are required");
    }
    // ── Constants ──
    ...
    const dailyProduction = effectiveSunHours * SYSTEM_EFFICIENCY * (1 - effectiveShadingLoss);
    ...
    const totalKw = Math.round(((actualPanelCount * PANEL_WATTS) / 1000) * 100) / 100;
    const estimatedAnnualKwh = Math.round(totalKw * annualProductionPerKw);
    const offsetPercentage = Math.min(Math.round((estimatedAnnualKwh / annualKwh) * 100), 120);
...
//# sourceMappingURL=cadService.js.map