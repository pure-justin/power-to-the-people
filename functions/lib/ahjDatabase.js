...
exports.getAhjStats = exports.verifyAhj = exports.getPermitSop = exports.createPermitSop = exports.updateAhjRequirements = exports.findAhjForAddress = exports.searchAhj = exports.getAhj = exports.createAhj = void 0;
...
async function requireAdmin(uid) {
    ...
    if (!userDoc.exists || ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.role) !== "admin") {
        throw new functions.https.HttpsError("permission-denied", "This action requires admin privileges.");
    }
}
// ─── createAhj ──────────────────────────────────────────────────────────────────
/**
...
exports.createAhj = functions
    ...
    if (!data.name || !data.state) {
        throw new functions.https.HttpsError("invalid-argument", "AHJ record requires at least 'name' and 'state' fields.");
    }
...
exports.getAhj = functions
    ...
    if (!data.ahjId) {
        throw new functions.https.HttpsError("invalid-argument", "ahjId is required.");
    }
    try {
        const doc = await admin
            ...
            .get();
        if (!doc.exists) {
            throw new functions.https.HttpsError("not-found", `AHJ with ID "${data.ahjId}" not found.`);
        }
...
exports.searchAhj = functions
    ...
    if (!data.state &&
        ...
        !data.zip_code &&
        !data.name) {
        throw new functions.https.HttpsError("invalid-argument", "At least one search filter is required (state, county, city, zip_code, or name).");
    }
    const db = admin.firestore();
    ...
    try {
        ...
        let query = db.collection("ahj_registry");
        if (data.state) {
            query = query.where("state", "==", data.state.toUpperCase());
        }
        if (data.county) {
 ...
 * @firestore ahj_registry
 */
exports.findAhjForAddress = functions
    .runWith({ timeoutSeconds: 30, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated.");
    ...
    if (!data.address || data.address.trim().length < 5) {
        throw new functions.https.HttpsError("invalid-argument", "A valid address string is required (must include ZIP code).");
    }
    try {
        ...
        if (!zipMatch) {
            throw new functions.https.HttpsError("invalid-argument", "Could not extract a ZIP code from the provided address. " +
        ...
        if (snapshot.empty) {
            ...
            return {
                ...
                candidates: [],
                zip_code: zipCode,
                message: `No AHJ found for ZIP code ${zipCode}. ` +
        ...
        const cityLevel = matches.filter((m) => m.jurisdiction_type === "city");
        const countyLevel = matches.filter((m) => m.jurisdiction_type === "county");
        if (cityLevel.length === 1) {
            ...
            return {
                ...
                zip_code: zipCode,
                message: "Matched city-level AHJ. County-level AHJ also covers this ZIP " +
        ...
        return {
            ...
            zip_code: zipCode,
            message: `Multiple AHJs (${matches.length}) cover ZIP ${zipCode}. ` +
 ...
 * @firestore ahj_registry
 */
exports.updateAhjRequirements = functions
    .runWith({ timeoutSeconds: 30, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated.");
    ...
    if (!data.ahjId || !data.requirements) {
        throw new functions.https.HttpsError("invalid-argument", "ahjId and requirements are required.");
    }
    const db = admin.firestore();
 ...
 */
exports.createPermitSop = functions
    ...
    if (!data.ahjId || !Array.isArray(data.steps) || data.steps.length === 0) {
        throw new functions.https.HttpsError("invalid-argument", "ahjId and a non-empty steps array are required.");
    }
    const db = admin.firestore();
    try {
 ...
 */
exports.getPermitSop = functions
    .runWith({ timeoutSeconds: 15, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be authenticated.");
...
exports.verifyAhj = functions
    ...
    if (!data.ahjId || !data.verifiedBy) {
        throw new functions.https.HttpsError("invalid-argument", "ahjId and verifiedBy are required.");
    }
    const db = admin.firestore();
    const docRef = db.collection("ahj_registry").doc(data.ahjId);
    try {
...
exports.getAhjStats = functions
    ...
    try {
        ...
        snapshot.docs.forEach((doc) => {
            ...
            if (((_a = data.portal) === null || _a === void 0 ? void 0 : _a.automatable) === true) {
                automatable++;
            }
        ...
        return {
            ...
            automatable,
            automatable_pct: total > 0 ? Math.round((automatable / total) * 100) : 0,
...
//# sourceMappingURL=ahjDatabase.js.map