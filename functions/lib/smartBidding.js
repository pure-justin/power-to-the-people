...
async function loadWeights() {
    try {
        ...
        return {
            price: typeof configData.price === "number"
                ? configData.price
                : DEFAULT_WEIGHTS.price,
            rating: typeof configData.rating === "number"
                ? configData.rating
                : DEFAULT_WEIGHTS.rating,
            proximity: typeof configData.proximity === "number"
                ? configData.proximity
                : DEFAULT_WEIGHTS.proximity,
            speed: typeof configData.speed === "number"
                ? configData.speed
                : DEFAULT_WEIGHTS.speed,
            availability: typeof configData.availability === "number"
                ? configData.availability
                : DEFAULT_WEIGHTS.availability,
            experience: typeof configData.experience === "number"
                ? configData.experience
                : DEFAULT_WEIGHTS.experience,
            reliability: typeof configData.reliability === "number"
 ...
 */
function scoreBid(bid, listing, worker, weights) {
    var _a;
    // --- Price ---
    ...
    // --- Rating ---
    const overallRating = (_a = worker.ratings) === null || _a === void 0 ? void 0 : _a.overall;
    const ratingScore = typeof overallRating === "number" && overallRating > 0
        ? clamp01(overallRating / 5.0)
        : 0.5;
    // --- Proximity ---
    const distanceMiles = typeof bid.worker_distance_miles === "number"
        ? bid.worker_distance_miles
        : MAX_DISTANCE_MILES;
    const proximityScore = clamp01(1 - Math.min(distanceMiles, MAX_DISTANCE_MILES) / MAX_DISTANCE_MILES);
    // --- Speed ---
    const proposedDays = typeof bid.proposed_days === "number"
        ? bid.proposed_days
        : MAX_PROPOSED_DAYS;
    const speedScore = clamp01(1 - Math.min(proposedDays, MAX_PROPOSED_DAYS) / MAX_PROPOSED_DAYS);
    // --- Availability ---
    const activeTasks = typeof worker.active_tasks === "number" ? worker.active_tasks : 0;
    const availabilityScore = clamp01(1 - Math.min(activeTasks, MAX_ACTIVE_TASKS) / MAX_ACTIVE_TASKS);
    // --- Experience ---
    ...
    const experienceScore = clamp01(Math.min(completedOfType, MAX_EXPERIENCE_JOBS) / MAX_EXPERIENCE_JOBS);
    // --- Reliability ---
    const reliabilityRaw = typeof worker.reliability_score === "number"
    ...
    const totalScore = breakdown.price * weights.price +
 ...
 */
async function rankBids(listingId) {
    var _a, _b;
    // Load listing
    ...
    bidsSnap.forEach((doc) => {
        ...
        const workerId = (_a = bidData.bidder) === null || _a === void 0 ? void 0 : _a.userId;
        if (workerId) {
    ...
    for (const { bidId, data: bidData } of bidsWithIds) {
        ...
        rankedBids.push({
            ...
            workerName: worker.name || ((_b = bidData.bidder) === null || _b === void 0 ? void 0 : _b.displayName) || "Unknown",
 ...
 */
async function autoAcceptBestBid(listingId) {
...
//# sourceMappingURL=smartBidding.js.map