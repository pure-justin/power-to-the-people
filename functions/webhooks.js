...
// In production, set this in Firebase config: firebase functions:config:set webhook.secret="your-secret"
...
exports.referralStatusWebhook = functions.https.onRequest(async (req, res) => {
  ...
  const validStatuses = ["signed_up", "qualified", "site_survey", "installed"];
 ...
 * }
 */
exports.referralBulkUpdateWebhook = functions.https.onRequest(
  async (req, res) => {
    if (req.method !== "POST") {
      res.status(405).json({ error: "Method not allowed" });
      return;
...
exports.referralStatsWebhook = functions.https.onRequest(async (req, res) => {
  ...
  const expectedKey = functions.config().webhook?.api_key || "development-key";
 ...
 * (Shared with referralFunctions.js - consider extracting to shared module)
 */
...
module.exports = {
  referralStatusWebhook: exports.referralStatusWebhook,
  referralBulkUpdateWebhook: exports.referralBulkUpdateWebhook,
  referralStatsWebhook: exports.referralStatsWebhook,
};
